...
import { ToolbarMenu, CardNavigatorSettings, NumberSettingKey } from '../../../common/interface';
import { SettingsManager } from '../../../presentation/views/settings/settingsManager';
import { FolderSuggest } from '../../../presentation/views/settings/components/FolderSuggest';
...
function updateSliderValue(section: HTMLElement, settingKey: keyof CardNavigatorSettings, plugin: CardNavigatorPlugin) {
    const settingItem = section.querySelector(`.setting-item[data-setting="${String(settingKey)}"]`);
    if (!settingItem) return;

    const slider = settingItem.querySelector('input[type="range"]');
    if (slider instanceof HTMLInputElement) {
        const value = plugin.settings[String(settingKey)] as number;
...
function updateToggleValue(section: HTMLElement, settingKey: keyof CardNavigatorSettings, plugin: CardNavigatorPlugin) {
    const toggle = section.querySelector(`.setting-item[data-setting="${String(settingKey)}"] input[type="checkbox"]`);
    if (toggle instanceof HTMLInputElement) {
        toggle.checked = plugin.settings[String(settingKey)] as boolean;
    }
}
...
function addDropdownSetting(
    ...
    new Setting(container)
        ...
        .addDropdown(dropdown => {
            ...
            dropdown.setValue(plugin.settings[String(key)] as string)
                .onChange(async (value) => {
                    await settingsManager.updateSetting(String(key), value);
                    if (onChange) {
                        onChange(value);
                    }
                });
...
function addToggleSetting(
    ...
    new Setting(container)
        ...
        .addToggle(toggle => toggle
            .setValue(plugin.settings[String(key)] as boolean)
            .onChange(async (value) => {
                await settingsManager.updateBooleanSetting(String(key), value);
                if (onChange) {
                    onChange();
                }
            })
...
function addSliderSetting(
    ...
    setting.settingEl.setAttribute('data-setting', String(key));
    ...
    const valueDisplay = createSpan({
        text: String(plugin.settings[String(key)] as number)
    });
    // 이름 요소 찾기
    ...
    setting.addSlider(slider => slider
        ...
        .setValue(plugin.settings[String(key)] as number)
        .onChange(async (value) => {
            if (
                (String(key) === 'bodyLength' && !plugin.settings.bodyLengthLimit) ||
                (String(key) === 'cardsPerView' && !plugin.settings.alignCardHeight)
            ) {
                return;
            }
            // 값 표시 업데이트
...
function onClickOutside(event: MouseEvent, windowObj: Window) {
    ...
    if (existingPopup && !existingPopup.element.contains(target) && !toolbarEl?.contains(target)) {
        if (
            existingPopup.type === 'sort' as ToolbarMenu ||
            (existingPopup.type === 'settings' as ToolbarMenu &&
