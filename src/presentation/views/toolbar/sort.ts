...
import { SortCriterion, SortOrder, SortOption } from '../../../domain/models/types';
import { getTranslatedSortOptions } from '../../../domain/models/types';
...
export function toggleSort(plugin: CardNavigatorPlugin, containerEl: HTMLElement | null) {
    ...
    // 이름 정렬 옵션 추가
    const nameOptions = sortOptions.filter((option: SortOption) => option.value.includes('fileName'));
    nameOptions.forEach((option: SortOption) => {
        menu.addItem(item => 
            item
    ...
    // 수정일 정렬 옵션 추가
    const modifiedOptions = sortOptions.filter((option: SortOption) => option.value.includes('lastModified'));
    modifiedOptions.forEach((option: SortOption) => {
        menu.addItem(item => 
            item
                .setTitle(option.label)
                .setChecked(currentSort === option.value)
    ...
    // 생성일 정렬 옵션 추가
    const createdOptions = sortOptions.filter((option: SortOption) => option.value.includes('created'));
    createdOptions.forEach((option: SortOption) => {
        menu.addItem(item => 
            item
                .setTitle(option.label)
                .setChecked(currentSort === option.value)
...
async function updateSortSettings(
    ...
    leaves.forEach(leaf => {
        if (leaf.view instanceof CardNavigatorView) {
            ...
            const searchService = plugin.searchService;
            ...
            if (resortedResults) {
                ...
                view.refreshBatch([RefreshType.CONTENT]);
            }
        }
    });
}
