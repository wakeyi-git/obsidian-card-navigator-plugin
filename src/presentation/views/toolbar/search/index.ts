...
import { MIN_SEARCH_TERM_LENGTH, SEARCH_DEBOUNCE_DELAY } from '../../../../domain/models/types';
import { View } from 'obsidian';
import { SearchOptions, DEFAULT_SEARCH_OPTIONS, BATCH_SIZE } from '../../../../domain/models/types';
...
// 검색 결과 표시
function displaySearchResults(plugin: CardNavigatorPlugin, query: string) {
    const leaves = plugin.app.workspace.getLeavesOfType(VIEW_TYPE_CARD_NAVIGATOR);
    leaves.forEach(leaf => {
        if (leaf.view instanceof CardNavigatorView) {
            const view = leaf.view;
            view.cardContainer.setSearchResults(null);
            view.cardContainer.displayCards([]);
        }
    });
}

// 검색 결과 업데이트
function updateSearchResults(plugin: CardNavigatorPlugin, results: TFile[]) {
    const leaves = plugin.app.workspace.getLeavesOfType(VIEW_TYPE_CARD_NAVIGATOR);
    leaves.forEach(leaf => {
        if (leaf.view instanceof CardNavigatorView) {
            const view = leaf.view;
            view.cardContainer.setSearchResults(results);
            view.cardContainer.displayCards(results);
        }
    });
}

// 검색 결과 초기화
function clearSearchResults(plugin: CardNavigatorPlugin) {
    const leaves = plugin.app.workspace.getLeavesOfType(VIEW_TYPE_CARD_NAVIGATOR);
    leaves.forEach(leaf => {
        if (leaf.view instanceof CardNavigatorView) {
            const view = leaf.view;
...
// 검색 결과 새로고침
function refreshSearchResults(plugin: CardNavigatorPlugin) {
    const leaves = plugin.app.workspace.getLeavesOfType(VIEW_TYPE_CARD_NAVIGATOR);
    leaves.forEach(leaf => {
        if (leaf.view instanceof CardNavigatorView) {
            const view = leaf.view;
