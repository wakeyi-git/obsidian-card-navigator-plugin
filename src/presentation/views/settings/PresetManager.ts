...
import { CardNavigatorSettings, Preset, globalSettingsKeys, DEFAULT_SETTINGS, GlobalSettingsKey } from '../../../domain/models/types';
...
export class PresetManager implements IPresetManager {
    ...
    async initialize(): Promise<void> {
        ...
        if (!await this.presetExists('default')) {
            const defaultPresetSettings = Object.fromEntries(
                Object.entries(DEFAULT_SETTINGS).filter(([key]) => !globalSettingsKeys.includes(key as GlobalSettingsKey))
            );
    ...
    private getCurrentSettings(): Partial<CardNavigatorSettings> {
        ...
        globalSettingsKeys.forEach(key => delete currentSettings[key as keyof CardNavigatorSettings]);
    ...
    async applyPreset(presetName: string): Promise<void> {
        ...
        if (preset) {
            const globalSettings = globalSettingsKeys.reduce((acc: Record<string, any>, key: string) => {
