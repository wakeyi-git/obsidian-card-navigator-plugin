...
import { getSearchService } from '@presentation/views/toolbar/search';
...
export class CardContainer {
    ...
    private async createCardsData(files: TFile[]): Promise<Card[]> {
        ...
        try {
            ...
            return cards.filter((card: Card | null) => card !== null);
        } catch (error) {
    ...
    private async renderCards(cardsData: Card[]) {
        ...
        // 변경된 카드만 업데이트하도록 최적화
        const changedCards = cardsData.filter((card: Card) => {
            const existingCard = this.cards.find(c => c.getFile().path === card.getFile().path);
            return !existingCard || existingCard.getFile().stat.mtime !== card.getFile().stat.mtime;
        });
    ...
    public async getFilteredFiles(): Promise<TFile[]> {
        ...
        if (this.cards.length > 0) {
            return this.cards.map(card => card.getFile());
        }
    ...
    public async searchCards(searchTerm: string) {
        ...
        if (this.cards.length > 0) {
            filesToSearch = this.cards.map(card => card.getFile());
        } else {
