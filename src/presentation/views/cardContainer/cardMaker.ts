...
import { CardContent, CardSection, CardSectionType } from '@domain/models/types';
...
export class CardMaker extends Component {
    ...
    public async createCard(file: TFile): Promise<Card> {
        try {
            ...
            const bodyContent = this.plugin.settings.showBody ? 
                this.truncateBody(this.removeFirstHeader(cleanBody)) : '';
    ...
    createCardElement(card: Card): HTMLElement {
        ...
        cardElement.setAttribute('data-card-id', card.getFilePath());
        cardElement.setAttribute('data-original-path', card.getFilePath());
    ...
    private addCardContent(cardElement: HTMLElement, card: Card) {
        ...
        // 헤더 섹션 처리
        content.header.forEach((header: CardSection) => {
            if (header.type === 'header') {
                this.createContentElement(cardElement, 'h3', header.content, 'filename', settings.fileNameFontSize);
            }
        });

        // 본문 섹션 처리
        content.body.forEach((body: CardSection) => {
            if (body.type === 'text') {
                const contentEl = this.createContentElement(cardElement, 'div', '', 'body', settings.bodyFontSize);
                
                if (settings.renderContentAsHtml) {
                    cardElement.dataset.content = body.content;
                    ...
                    contentEl.textContent = body.content;
