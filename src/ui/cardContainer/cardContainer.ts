...
export class CardContainer {
    ...
    async initialize(containerEl: HTMLElement) {
        ...
        try {
            ...
            console.log('[CardNavigator] 뷰 상태:', isNewView ? '새 뷰' : '기존 뷰');
            
            // 초기화 중 플래그 설정 - 중복 렌더링 방지
            this.isDisplayingCards = true;
            
            // 컨테이너 크기 대기 로직
            console.log('[CardNavigator] 컨테이너 크기 대기 시작');
            try {
                // 컨테이너 크기 대기 (개선된 메서드 사용)
                await this.waitForContainerSize();
                console.log('[CardNavigator] 컨테이너 크기 대기 완료');
            } catch (error) {
                console.warn('[CardNavigator] 컨테이너 크기 대기 중 문제 발생, 기본 크기로 계속 진행합니다');
            }
            ...
            // 레이아웃 매니저 초기화
            this.layoutManager = new LayoutManager(this.plugin, containerEl, this.cardMaker, this);
            console.log('[CardNavigator] 레이아웃 매니저 초기화 완료');
            ...
            const isLayoutManagerInitialized = !!this.layoutManager && 
                typeof this.layoutManager.getLayoutStrategy === 'function' &&
                typeof this.layoutManager.setLayout === 'function';
            
            // 초기화 완료 이벤트 발생 - 레이아웃 설정 전에 발생시켜 중복 렌더링 방지
            ...
            console.log('[CardNavigator] 컨테이너 초기화 완료 이벤트 발생, 레이아웃 매니저 초기화 상태:', isLayoutManagerInitialized);
            
            // 초기 레이아웃 설정 (약간 지연시켜 실행)
            console.log('[CardNavigator] 초기 레이아웃 설정 지연 시작');
            
            // 새 뷰인 경우 지연 시간 단축
            const layoutDelay = isNewView ? 10 : 100;
            
            setTimeout(() => {
                console.log('[CardNavigator] 초기 레이아웃 설정 시작');
                this.setLayout(this.plugin.settings.defaultLayout);
                console.log('[CardNavigator] 초기 레이아웃 설정 완료');
            }, layoutDelay);
            
            console.log('[CardNavigator] CardContainer.initialize 완료');
            return this;
        } catch (error) {
            console.error('[CardNavigator] 컨테이너 초기화 중 오류 발생:', error);
            // 오류가 발생해도 이벤트는 발생시켜 UI가 멈추지 않도록 함
            if (this.containerEl) {
                // 레이아웃 매니저 초기화 상태 확인
                const isLayoutManagerInitialized = !!this.layoutManager && 
                    typeof this.layoutManager.getLayoutStrategy === 'function' &&
                ...
                this.containerEl.dispatchEvent(event);
                console.log('[CardNavigator] 오류 발생 후 컨테이너 초기화 완료 이벤트 발생, 레이아웃 매니저 초기화 상태:', isLayoutManagerInitialized);
            }
            // 초기화 중 플래그 해제
            this.isDisplayingCards = false;
            throw error;
    ...
    /**
     * 초기 컨테이너 크기를 설정하고 방향을 결정합니다.
     * 이 메서드는 initialize 과정에서 호출됩니다.
     */
    private initializeContainerSizeAndOrientation(): void {
        ...
        if (this.containerEl.offsetWidth > 0 && this.containerEl.offsetHeight > 0) {
            ...
            // 레이아웃 스타일 매니저에 초기 크기 전달
            if (this.layoutStyleManager) {
                this.layoutStyleManager.setInitialContainerSize(this.lastWidth, this.lastHeight);
            }
        } else {
            // 리프 크기 기반으로 초기 컨테이너 크기 설정
            this.setInitialSizeFromLeaf();
        }
        
        // 방향 결정
        this.isVertical = this.determineVerticalOrientation();
        console.log('[CardNavigator] 컨테이너 방향 결정:', this.isVertical ? '세로' : '가로');
    }
    ...
    /**
     * 지정된 파일 목록으로 카드를 표시합니다.
     * 이미 렌더링 중인 경우 대기 요청으로 설정하고,
     * 파일 목록이 변경되지 않은 경우 활성 카드만 업데이트합니다.
     * @param displayFiles 표시할 파일 목록
     */
    public async displayCards(displayFiles: TFile[]): Promise<void> {
        console.log('[CardNavigator] displayCards 호출됨, 파일 수:', displayFiles.length);
        ...
        try {
            // 파일 목록이 변경되지 않았고 카드가 이미 있는 경우 활성 카드만 업데이트
            if (this.cards.length > 0 && this.areFileListsEqual(displayFiles, this.cards.map(card => card.file))) {
                console.log('[CardNavigator] 파일 목록이 변경되지 않음, 활성 카드만 업데이트');
                const activeFile = this.plugin.app.workspace.getActiveFile();
                if (activeFile && this.cardRenderer) {
                    this.cardRenderer.updateActiveCard(activeFile, this.focusedCardId);
                }
                return;
            }
            
            // 파일 목록이 변경된 경우에만 카드 데이터 새로 생성
            console.log('[CardNavigator] 카드 데이터 생성 시작');
            const cardsData = await this.createCardsData(displayFiles);
            console.log('[CardNavigator] 카드 데이터 생성 완료, 카드 수:', cardsData.length);
            
            if (cardsData.length === 0) {
                console.log('[CardNavigator] 생성된 카드 데이터 없음');
            ...
            console.log('[CardNavigator] 카드 렌더링 시작');
            await this.renderCards(cardsData);
            console.log('[CardNavigator] 카드 렌더링 완료');
        } finally {
            // 이전 상태로 복원 (중첩 호출 처리)
            this.isDisplayingCards = wasDisplayingCards;
            console.log('[CardNavigator] 렌더링 상태 초기화:', wasDisplayingCards ? '여전히 렌더링 중' : '렌더링 완료');
            
            // 대기 중인 요청이 있으면 처리
            this.processPendingDisplayRequest();
        }
    }
    ...
    private async renderCards(cardsData: Card[]) {
        console.log('[CardNavigator] renderCards 호출됨, 카드 수:', cardsData.length);
        ...
        console.log('[CardNavigator] 변경된 카드 수:', changedCards.length, '정렬 변경 여부:', orderChanged);
        
        // 변경사항이 없고 강제 렌더링이 필요하지 않은 경우 렌더링 스킵
        if (changedCards.length === 0 && this.cards.length === cardsData.length && !orderChanged && !this.pendingDisplayRequest) {
            console.log('[CardNavigator] 변경사항 없음, 렌더링 스킵');
        ...
        const activeFile = this.plugin.app.workspace.getActiveFile();
        console.log('[CardNavigator] 활성 파일:', activeFile?.path || '없음');

        try {
            // 카드 렌더링
            console.log('[CardNavigator_cardContainer_renderCards] cardRenderer.renderCards 호출');
            ...
            requestAnimationFrame(() => {
                ...
                console.log('[CardNavigator] 활성 카드 인덱스:', newActiveCardIndex);
                
                if (newActiveCardIndex !== -1) {
                    // 약간의 지연 후 스크롤 실행
                    console.log('[CardNavigator] 활성 카드로 스크롤 예약');
                    setTimeout(() => {
                        this.scrollToActiveCard(false);
                        console.log('[CardNavigator] 활성 카드로 스크롤 완료');
                    }, 50);
                }
            });
        } catch (error) {
        ...
        console.log('[CardNavigator] renderCards 완료');
    }
    //#endregion
