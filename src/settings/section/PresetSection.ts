 ...
 * 카드 프리셋 관련 설정을 관리합니다.
 */
export class CardPresetSection extends SettingSection {
  ...
  private async saveCurrentSettingsAsPreset(): Promise<void> {
    ...
    const presetSettings: any = {
      ...
      layout: {
        fixedCardHeight: settings.layout?.fixedCardHeight || true,
        layoutDirectionPreference: settings.layout?.layoutDirectionPreference || LayoutDirectionPreference.AUTO,
        cardMinWidth: settings.layout?.cardMinWidth || 200,
        cardMaxWidth: settings.layout?.cardMaxWidth || 400,
        cardMinHeight: settings.layout?.cardMinHeight || 150,
        cardMaxHeight: settings.layout?.cardMaxHeight || 600,
